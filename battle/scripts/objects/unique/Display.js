Display = {
	...
	state : {
		...
		transition : function (state, track, original) {
			...
			if (to === null)
				throw "You've tried to transition to an older state than the current one!";
			if (arguments.length < 2) {
				track = {
					completed : false
				};
			...
			for (var i = 0; i < mostBattlers; ++ i) {
				...
				foreach(["health", "experience"], function (property) {
					var difference = (toAll[i][property] - originalAll[i][property]) * ((1 / Settings.statTransitionDuration) / Time.framerate);
				...
				foreach(["transition", "height"], function (property) {
					var difference = (toAll[i].battler.display[property] - originalAll[i].battler.display[property]) * ((1 / Settings.switchTransitionDuration) / Time.framerate);
